...
def main():
    ...
    while game_number < number_of_games:
        ...
        if sum(swapper.get_dice()) == sum(loaded_dice.get_dice()):
            #print("Draw!")
            pass
        elif sum(swapper.get_dice()) > sum(loaded_dice.get_dice()) and sum(swapper.get_dice()) > sum(doubler.get_dice()):
            #print("Dice swapper wins!")
            swapper_score+= 1
        elif sum(doubler.get_dice()) > sum(loaded_dice.get_dice()) and sum(doubler.get_dice()) > sum(swapper.get_dice()):
                doubler_score+=1
        elif sum(loaded_dice.get_dice()) > sum(swapper.get_dice()) and sum(loaded_dice.get_dice()) > sum(doubler.get_dice()):
            loaded_dice_score +=1
